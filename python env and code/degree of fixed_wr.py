...
def find_fixed_mean_sd(N, p, S, d, mode = "hard", replace = False, seed = 1):
    ...
    return mean_t, var_t, sd_t

N = 200; p = 0.1; d = 1; seed = 200; S = 20; mode = "soft"
mytime = range(N-2)
mymean_wr, myvar_wr, mysd_wr = find_fixed_mean_sd(N, p, S, d, mode = mode, replace = True, seed = seed)
...
plt.subplot(1,3,2)
plt.plot(mytime, myvar_wr, color = "red")
plt.xlabel("t"); plt.ylabel("variance of D_i(t)")
plt.subplot(1,3,3)
plt.plot(mytime, mysd_wr, color = "blue")
plt.xlabel("t"); plt.ylabel("standard deviation of D_i(t)")
plt.savefig(fname = "degree of fixed wr.png", dpi = 800)
