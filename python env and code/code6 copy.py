...
def diffd(S, N, d, seed):
  ...
  inputs = range(0, S)
  mylist1 = []; mylist2 = []
  mylist1 = Parallel(n_jobs = -1)(delayed(find_maximal)(N, d = d, mode = "soft_fixed", replace = False, seed = (seed+i+1)) for i in inputs)
  mylist2 = Parallel(n_jobs = -1)(delayed(find_maximal)(N, d = d, mode = "soft_fixed", replace = True, seed = (seed+i)) for i in inputs)
...
listd = np.array([5, 15], dtype = int)
for i in listd:
  diffd(S = 10, N = 100, d = i, seed = 10*i)
...
This message is sent from Python. Time taken is {} seconds starting from {} to {}.""".format(figname, round(t_end - t_start), t_start, t_end)
